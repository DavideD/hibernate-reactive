		...
		try {
			...
			factory.withSession(
					...
					session -> session.find( Book.class, book1.getId() )
							// print its title
							.invoke( book -> System.out.println( book.getTitle() + " is a great book!" ) )
			)
					.await().indefinitely();
			...
			factory.withSession(
					...
					session -> session.find( Author.class, author1.getId(), author2.getId() )
							.invoke( authors -> authors.forEach( author -> System.out.println( author.getName() ) ) )
			)
					.await().indefinitely();
			...
			factory.withSession(
					...
					session -> session.find( Author.class, author2.getId() )
							...
							.chain( author -> fetch( author.getBooks() )
									// print some info
									.invoke( books -> {
										System.out.println( author.getName() + " wrote " + books.size() + " books" );
			...
			factory.withSession(
					...
					session -> session.find( Book.class, book1.getId() )
							...
							.chain( book -> fetch( book.getAuthor() )
									...
									.invoke( author -> System.out.printf( "%s wrote '%s'\n", author.getName(), book1.getTitle() ) )
							)
			)
					.await().indefinitely();
...
//									.invoke( published -> System.out.printf(
...
//									.invoke( coverImage -> System.out.println( new String( coverImage ) ) )
//							)
//					)
//					.await().indefinitely();
